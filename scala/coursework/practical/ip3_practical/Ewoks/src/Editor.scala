...
class Editor {
    ...
    def transpose() : Boolean = {
        var p = ed.point
        if (p == 0 || p == ed.length) {beep(); return false}
        ed.swapChar(p)
        ed.point += 1
        return true 
    ...
    def placemark() {ed.mark = ed.point }
    def swapmark() {
        var tmp = ed.mark 
        ed.mark = ed.point
        ed.point = tmp 
    ...
    var str= ""
    def copy() {
        var mn = ed.point.min(ed.mark)
        var mx = ed.point.max(ed.mark)
        str = ed.copy(mn,mx-mn+1)
    }
    def paste() {
        ed.insert(ed.point,str)
    }
    def cut() {
        copy()
        ed.deleteRange(ed.point,str.length)
    }
...
object Editor {
    ...
    val keymap = Keymap[Command](
        ...
        Display.ctrl('T') -> (_.transpose),
        Display.ctrl('M') -> (_.placemark),
        Display.ctrl('O') -> (_.swapmark),
