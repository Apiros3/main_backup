...
class Editor {
    ...
    //Command for Q3.1.2 in swapping
    def swap() : Boolean {
        var p = ed.point 
        if (p == ed.length || p == 0) { beep(); return false }
        ed.swapChar(p)
        ed.point += 1 
        return true 
    }
    //Command for Q3.1.3 in marking and swapping marks
    def mark() {
        ed.mark = ed.point 
    }
    def swap_mark() {
...
object Editor {
    ...
    val keymap = Keymap[Command](
        ...
        //Q3.1.2
        Display.ctrl('T') -> (_.swap),
        //Q3.1.3
        Display.ctrl('M') -> (_.mark)
        Display.ctrl('O') -> (_.swap_mark)
